...
description: >
  ...
  strategy discovery workspace, including assigned per-ticker research tasks in
  an existing `abel-invest` workspace.
metadata:
  openclaw:
    requires:
      bins:
...
6. On a fresh skill install where `abel-invest` is not available yet, use
...
13. Do not call parameter, sizing, threshold, filter, or window tweaks broad
     exploration.
14. Every recorded round requires an agent-written `research_journal.md` entry
     with the round ledger reference before the next recorded round.
15. Treat input realization as an evidence fact: a graph-supported declaration
...
16. Create new sessions only after workspace context resolves. Do not use
     `--root` unless intentionally creating a legacy/offline session, and then
     pass `--allow-outside-workspace`.
17. Do not create an online session view automatically. If a candidate round
...
18. The default Abel router base URL is `https://api.abel.ai/router/`.
...
19. The framework defines evidence validity. The agent owns the strategy
     thinking.
