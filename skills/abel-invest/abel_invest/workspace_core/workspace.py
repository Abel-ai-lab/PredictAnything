...
This command is read-only: it prepares the stop-report payload without
...
read-only and does not export, upload, or promote artifacts. Report its selected
strategy exactly instead of manually walking `results.tsv` or branch folders to
choose the best session strategy. Ask the user whether to create a session
review page if a recorded candidate exists. If the user agrees or explicitly
asks to publish the session review page, run
